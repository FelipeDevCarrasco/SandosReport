 ...
 * @param {string} outputFilePath - Ruta donde guardar el archivo (opcional)
 * @param {function} onProgress - Callback de progreso (percentage, message) (opcional)
 */
export async function generarReporteExcelVentas(query = '', fechaDesde = '', fechaHasta = '', outputFilePath = null, onProgress = null) {
  ...
  for (let i = 0; i < totalRefs; i += BATCH_SIZE) {
    const batch = referenciasUnicas.slice(i, i + BATCH_SIZE);
    const progressPct = 20 + Math.floor((i / totalRefs) * 70);
    reportProgress(progressPct, `Consultando Sphinx: ${i + 1}-${Math.min(i + BATCH_SIZE, totalRefs)} de ${totalRefs}...`);
    ...
    batchResults.forEach(rows => todasLasFilas.push(...rows));
  }
  ...
  if (outputFilePath) {
    ...
    const timestamp = generateTimestamp();
    outputFileName = `reporte_combinado_${timestamp}.xlsx`;
    outputPath = path.join(getUploadsDir(), outputFileName);
  }

  escribirExcel(todasLasFilas, columns, outputPath, true);
  reportProgress(100, 'Reporte generado exitosamente');
