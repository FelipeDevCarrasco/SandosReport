...
async function callReporteService(param, intento = 1) {
  ...
  if (isSesionExpirada(data) && intento === 1) {
    ...
    try {
      const { loginSphinxWithPuppeteer, verificarSesion } = await import('./sphinx-login.service.js');
      const nuevaSesion = await loginSphinxWithPuppeteer();
      
      console.log('🔍 Verificando nueva sesión...');
      const esValida = await verificarSesion(nuevaSesion);
      
      if (!esValida) {
        console.warn('⚠️ La nueva sesión no es válida, reintentando login...');
        await new Promise(r => setTimeout(r, 2000));
        const segundoIntento = await loginSphinxWithPuppeteer();
        setSession(segundoIntento);
      } else {
        setSession(nuevaSesion);
      }
      
      console.log('⏳ Esperando antes de reintentar consulta (2s)...');
      await new Promise(r => setTimeout(r, 2000));
      
      return callReporteService(param, 2);
    } catch (err) {
      console.error('❌ No se pudo renovar sesión automáticamente:', err.message);
