...
/**
 * GET /api/report/excel?query=
 * Genera reporte Excel combinado (Sphinx + Shipit) para las ventas que coinciden con el query
 */
export async function generateExcelReport(req, res) {
  try {
    ...
    const { outputPath, outputFileName } = await generarReporteExcelVentas(query || '');
